...
foreach ($xpath->evaluate('//s:url') as $url) {
    ...
    $id_page = Query("SELECT id_page FROM BWS_Traduction WHERE (it, en) = ('$data[it]', '$data[en]')")->fetch_array(MYSQLI_ASSOC)['id_page'];
    ...
    if (!$id_page) {
        ...
        Query("INSERT INTO BWS_Traduction (id_page, it, en) VALUES ($id_page, '$data[it]', '$data[en]')");
