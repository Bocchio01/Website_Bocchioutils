...
if ($login) {
    ...
    $updated_pages = Query("SELECT CONCAT(p.name, ' - (', p.type, ')') as name_link , CONCAT('$host', t.it) as it, CONCAT('$host', t.en) as en FROM BWS_Pages AS p JOIN BWS_Translations AS t WHERE p.id_page = t.id_page AND YEAR(p.last_modify) = YEAR(CURRENT_DATE()) AND MONTH(p.last_modify) = MONTH(CURRENT_DATE() - INTERVAL 1 MONTH) AND p.type != 'Undefined' AND p.type != 'Index' ORDER BY p.id_page");

    print_r($new_pages);
    print_r($updated_pages);

    if ($new_pages->num_rows) {
        foreach ($lang as $l) $msg_new[$l] = "";
    ...
    if ($updated_pages->num_rows) {
        foreach ($lang as $l) $msg_updated[$l] = "";
